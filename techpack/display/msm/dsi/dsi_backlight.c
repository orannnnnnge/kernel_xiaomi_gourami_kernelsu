...
static int dsi_panel_wled_bl_register(struct dsi_backlight_config *bl);
...
static int dsi_backlight_update_wled(struct dsi_backlight_config *bl, u32 bl_lvl)
{
	int rc = 0;
	...
	if (rc < 0)
		pr_err("failed to update wled backlight:%d\n", bl_lvl);
...
int dsi_panel_bl_register(struct dsi_panel *panel)
{
	...
	if (!register_func || (rc == -ENOTSUPP)) {
		...
		case DSI_BACKLIGHT_WLED:
			register_func = dsi_panel_wled_bl_register;
			break;
...
static int dsi_panel_wled_bl_register(struct dsi_backlight_config *bl)
{
	...
	rc = dsi_backlight_register(bl);
	if (rc) {
		pr_err("[%s] failed to parse wled backlight, rc=%d\n",
		       panel->name, rc);
	}

	bl->raw_bd = bd;
	bl->update_bl = dsi_backlight_update_wled;
	return rc;
}
